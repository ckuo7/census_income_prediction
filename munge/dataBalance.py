...
def dataUnbalance(y,percent,ratio):
    ...
    if percent <= 0.05:
        ...
        pos_train_index = np.random.choice(positive_indexes,training_size,replace=True)
        neg_train_index = np.random.choice(negative_indexes,training_size*ratio,replace=False)
    else:
        pos_train_index = np.random.choice(positive_indexes,training_size,replace=True)
        neg_train_index = np.random.choice(negative_indexes,training_size*ratio,replace=True)
